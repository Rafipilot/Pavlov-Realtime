...
def c0_instinct_rule(INPUT, Agent):
    ...
    if INPUT[0] == 1    and     Agent.buffer[ 0,  Agent.arch.Z__flat[0]]==1 and type== "Pleasure":        # self.Z__flat[0] needs to be adjusted as per the agent, which output the designer wants the agent to repeat while learning postively or negatively
...
def c1_instinct_rule(INPUT, Agent):
    ...
    if INPUT[10] == 1 and type == "Fear":
        ...
        if Agent.buffer[ 0,  Agent.arch.Z__flat[0]] == 1: #If reacted without Food or Fear 

            if Agent.counter ==0:
                instinct_response = [1, "c1 instinct triggered"]
                print("applying neg signal")
                Agent.counter = number_qa_neurons
            else:
                instinct_response = [0, "c1 pass"]            
...
def qa0_firing_rule(INPUT, Agent): 
    ...
    if Agent.counter < (number_qa_neurons+1) and INPUT[0] == 1    and Agent.buffer[ 0,  Agent.arch.Z__flat[0]] == 1: #If the agent ate food 

        Agent.counter += 1
        group_response = np.zeros(number_qa_neurons)
        group_response[0 : Agent.counter] = 1

    elif INPUT[0] == 0    and Agent.buffer[ 0,  Agent.arch.Z__flat[0]] == 1:   
        if Agent.counter == 0:
            ## pain signal needed
